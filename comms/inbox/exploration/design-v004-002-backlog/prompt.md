...
This is part of Phase 1 (Logical Design) for `auto-dev-test-target-1` version `v004`.
...
- Call `get_backlog_item(project="auto-dev-test-target-1", item_id="BL-XXX")`
...
Call `list_versions(project="auto-dev-test-target-1")` to find the most recent completed version before `v004`.
...
- Learnings that apply to `v004` scope
...
Run `search_learnings(project="auto-dev-test-target-1", tags=["design", "architecture"])` to find relevant documented learnings.

Review any learnings that might inform the design approach for `v004`.
