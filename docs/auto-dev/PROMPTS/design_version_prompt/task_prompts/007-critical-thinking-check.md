...
This is the Critical Thinking Check between Phase 3 and Phase 4 for `${PROJECT}` version `${VERSION}`.

Documents have been persisted to `comms/inbox/versions/execution/${VERSION}/`. Now review and enhance them.
...
```bash
grep "^- [0-9][0-9][0-9]-.*:" comms/inbox/versions/execution/${VERSION}/THEME_INDEX.md
```
...
Verify `comms/inbox/versions/execution/${VERSION}/` exists.
...
Create `v${VERSION}-phase-1-logical-design-draft.md`:
...
Create `v${VERSION}-phase-1-logical-design-risks.md` listing:
...
- Use DeepWiki to search GitHub repos for examples
- Use explore tool for parallel investigations (allowed with `allowed_mcp_tools=["ALL_ALLOWED"]`)

Document findings in `v${VERSION}-phase-1-logical-design-risks-investigation.md`.

### Step 8: Create Final Design

Merge draft and investigation results into `v${VERSION}-phase-1-logical-design.md`:
...
**VERIFICATION GATE** (create `v${VERSION}-step9-verification.md`):
...
```bash
git add comms/inbox/versions/execution/${VERSION}/
git add comms/outbox/exploration/design-${VERSION}-007-critical-check/
git commit -m "design: enhance v${VERSION} documents with critical thinking analysis"
git push
```

## Output Requirements

Create in `comms/outbox/exploration/design-${VERSION}-007-critical-check/`:

### README.md (required)

First paragraph: Summary of critical thinking check for v${VERSION}.
...
### v${VERSION}-phase-1-logical-design.md

Final comprehensive logical design.

### v${VERSION}-phase-1-logical-design-risks-investigation.md

Investigation findings for all identified risks.

### v${VERSION}-step9-verification.md
...
```bash
git add comms/outbox/exploration/design-${VERSION}-007-critical-check/
git commit -m "exploration: design-${VERSION}-007-critical-check complete"
git push
```
